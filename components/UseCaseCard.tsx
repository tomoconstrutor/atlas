...
export function UseCaseCard({ useCase, locale }: UseCaseCardProps) {
  ...
  return (
    <article className="rounded-2xl bg-mind-surface p-4 shadow-mindSm transition hover:-translate-y-1 hover:shadow-mindMd">
      ...
      <h4 className="font-display text-2xl uppercase leading-none text-mind-ink">{title}</h4>

      <p className="mt-3 text-sm font-light leading-6 text-mind-muted">{description}</p>

      <div className="mt-4 border-t border-[var(--color-rule)] pt-3 text-sm font-light leading-6 text-mind-ink">
        {value}
      </div>
    </article>
  );
